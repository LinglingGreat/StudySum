...
创建并切换到新的本地分支 key_dev，同时设置它跟踪远程的 key_dev 分支：
    
    `git checkout -b key_dev gitlab/key_dev`
...
2. 切换到master主分支上 `git checkout master`

3. 将master更新的代码拉取到本地 `git pull`

4. 再切换到自己的分支假设为： add_order上 `git checkout add_order`

5. 合并master到自己的分支add_order上  `git merge master`

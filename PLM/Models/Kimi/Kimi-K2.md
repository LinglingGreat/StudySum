...
- Chunk-wise autoregressive generation：**为了保持全局一致性并避免长文档中的信息丢失**（幻觉），采用了基于块的自回归重写策略。文本被分成几个片段，单独改写，然后重新拼接在一起形成完整的段落
...
- **多轮交互**：模拟用户（不同的特质、偏好等）、工具、环境的完整交互链
...
- 多源指令生成
	...
	2. 受 AutoIF（Qwen发的一篇论文，通过种子任务拓展新任务） 启发的代理式指令增强。
...
- 工程题：爬取 GitHub PR/issue，构建带可执行测试的真实开发环境；Kubernetes 沙盒支持 1 万并发实例。（可能是Kimi-RL的核心）
...
- **PTX 损失**：从预训练中手工选择的高质量样本，防止遗忘高质量知识
...
- 如果有个环境很吃算力，单独部署提供API
- 多轮交互，每条数据的轮次不同，只要有一部分结束了整个就结束
- 用OpenAI Gym框架去交互

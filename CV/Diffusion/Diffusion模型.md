...
训练：
1.  从训练数据中加载一些图像
2.  添加不同级别的噪声。我们希望模型在面对添加了极端噪声和几乎没有添加噪声的带噪图像时，都能够很好地估计如何 “修复”（去噪）。
3.  将带噪输入送入模型中
4.  评估模型对这些输入进行去噪的效果(计算模型预测和加噪前的图像之间的损失)
5.  使用此信息更新模型权重

推理：从完全随机的输入开始，反复将其输入模型，每次根据模型预测进行少量更新，最终得到一张图像。有许多采样方法试图简化这个过程。

# -*- coding: utf-8 -*-
# @Author: LiLing
# @Date:   2018-09-11 09:13:23
# @Last Modified by:   Liling
# @Last Modified time: 2018-09-11 10:48:10
"""
使用朴素贝叶斯进行文档分类
...
朴素贝叶斯分类器通常有两种实现方式：一种基于贝努利模型实现，
一种基于多项式模型实现。
基于贝努利的实现方式并不考虑词在文档中出现的次数，只考虑出不出现，这是词集模型(set-of-words model)
考虑词在文档中的出现次数，是词袋模型(bag-of-words model)

三个例子：
过滤网站的恶意留言
过滤垃圾邮件
从不同区域里个人发布的广告中获取区域倾向
...
def trainNB0(trainMatrix,trainCategory):
    ...
    for i in range(numTrainDocs):
        if trainCategory[i] == 1:
            ...
            p1Denom += sum(trainMatrix[i])   # 该文档的总词数
        else:
            p0Num += trainMatrix[i]
...
# 便利函数，封装所有操作
def TestingNB():
    listOPosts,listClasses = loadDataSet()    # 数据集和对应标签
    myVocabList = createVocabList(listOPosts)    # 不重复的词汇表
    trainMat=[]
    for postinDoc in listOPosts:
        trainMat.append(setOfWords2Vec(myVocabList, postinDoc))    # 得到数据集的向量表示矩阵
    p0V,p1V,pAb = trainNB0(np.array(trainMat),np.array(listClasses))     # 计算类别概率和先验概率
    # 测试集
...
def spamTest():
    ...
    for i in range(1,26):
        ...
        fullText.extend(wordList)
        classList.append(0)
    vocabList = createVocabList(docList)   # create vocabulary不重复词汇表
...
# RSS源分类器及高频词去除函数,返回在fullText出现频率最高的30个单词
...
# 使用两个RSS源作为参数，获取文本，训练并测试，返回词汇表和类别属性先验概率

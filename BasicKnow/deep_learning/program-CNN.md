...
输出**Z：(m, n_H, n_W, n_C) **       其中 n_H = 1 + int((n_H_prev + 2 * pad - f) / stride)，n_W = 1 + int((n_W_prev + 2 * pad - f) / stride)

经过激活函数后**A：(m, n_H, n_W, n_C) ** （这里没有）
...
输出**A：(m, n_H, n_W, n_C) **       其中 n_H = int(1 + (n_H_prev - f) / stride)，n_W = int(1 + (n_W_prev - f) / stride)，n_C = n_C_prev
